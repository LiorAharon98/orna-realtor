    ...
    <div id={toggleSideBar ? styles.page_container_active : styles.page_container_inactive}>
      {toggleSideBar && (
        <div id={styles.link_container}>
          ...
          {JSON.parse(localStorage.getItem("orna")) && (
            <>
              <Link onClick={handleClick} className={styles.link} to={"/add-property"}>
                הוספה
              </Link>
              <Link onClick={handleClick} className={styles.link} to={"/admin"}>
                ניהול
              </Link>
            </>
          )}
